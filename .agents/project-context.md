...
Frontend is a static React SPA suitable for hosting under a subdomain such as
`tabortmig.jakob.nu`. Backend services (authentication, storage, scheduled
reminders, and email sending) are provided by Firebase (Firestore, Auth,
Cloud Functions + Cloud Scheduler) and an external email provider.
...
- `src/pages/`
  - Owns top-level route components (e.g. `HomePage`, `SitesPage`, `AccountPage`).
...
- `components` must not import from `pages` or routing; they are presentation
  only.
- Cross-folder imports should reflect these ownership rules and avoid circular
  dependencies.

## Routing & pages

TaBortMig is a single-page React app using React Router v6. The exact pages and
routes will be refined during the MVP definition, but the general pattern is:

- A landing/home view that explains the service.
- A view that lists supported sites with links and controls for opt-out dates.
- A simple account view where users can see/update their email and opt-out
  data.
...
- **scope** aligns with the owning area: `sites`, `optOuts`, `pages`,
  `components`, `data`, `space`, etc.
